...
void FBANodeActions::SwapNodeInDirection(EEdGraphPinDirection Direction)
{
	...
	{
		// Knot node linked to PinInDirection
		TArray<UEdGraphPin*> LinkedToPinInDirection = PinInDirection.GetPin()->LinkedTo;
		if (LinkedToPinInDirection.Num() > 0)
		{
			if (UK2Node_Knot* KnotNode = Cast<UK2Node_Knot>(LinkedToPinInDirection[0]->GetOwningNode()))
			{
				// UE_LOG(LogTemp, Warning, TEXT("Found knot PinInDirection")); 
				UEdGraphPin* PinOppD = FBAUtils::GetKnotPinByDirection(KnotNode, OppositeDirection);
				if (PinAInDirection)
				{
					PendingConnections.Add(FPinLink(PinAInDirection, PinOppD));
				}
				Schema->BreakSinglePinLink(PinInDirection.GetPin(), PinOppD);
			}
		}
	}
	...
	if (PinA.IsValid())
	{
		// Knot node linked to PinA
		TArray<UEdGraphPin*> LinkedToA = PinA.GetPin()->LinkedTo;
		if (LinkedToA.Num())
		{
			if (UK2Node_Knot* KnotNode = Cast<UK2Node_Knot>(LinkedToA[0]->GetOwningNode()))
			{
				// UE_LOG(LogTemp, Warning, TEXT("Found knot PinA"));
				UEdGraphPin* KnotPinInDir = FBAUtils::GetKnotPinByDirection(KnotNode, Direction);
				PendingConnections.Add(FPinLink(PinOpposite.GetPin(), KnotPinInDir));
				Schema->BreakSinglePinLink(PinA.GetPin(), KnotPinInDir);
			}
		}
	}
